...
const resolve = dir => require('path').join(__dirname, dir); //解析模块规则
...
const config = {
  ...
  output: {
    ...
    filename: isDev ? '[name].bundle.js' : 'js/[name].[chunkhash:10].js', //文件名称+目录
    path: path.join(__dirname, '../dist'), //输出文件目录（所有资源输出的公共目录）
    // publicPath: '/', //所有资源引入公共路径前缀   img/a.jpg ---> /img/a.jpg
    chunkFilename: 'js/[name]_chunk.[chunkhash:10].js', //非入口chunk的名称 import引入的js
  ...
  resolve: {
    ...
    //配置省略文件路径的后缀名  /js/index.js ---> /js/index  按照js-json-css顺序查找
    // extensions: ['.js', '.json', '.jsx', '.css'],
    
    //告诉webpack解析模块去哪个目录查找会逐级查找
  ...
  module: {
    rules: [
      ...
      },
      {
        ...
        // enforce: 'pre', //pre--->优先执行    post--->延后执行  normal--->正常处理（默认） inline--->其次处理
        use: [
          /** 
           * thread-loader  开启多进程
  ...
  plugins: [
    ...
    new AddAssetHtmlWebpackPlugin({
      filepath:require.resolve('../dist/dll/jquery.js')
      // filepath: resolve(__dirname, '../dist/dll/jquery')
    })
